...
float rackColor[3] = {0.7f, 0.7f, 0.7f};
float componentsAngleY = 0.0f;
...
float rackPositionX = -2.0f;
float rackSpeed = 0.02f;
...
float gearSpeeds[4] = {1.0f, 1.0f, 1.0f, 1.0f};
bool arrowPointingLeft = false;
bool isAnimating = false;
bool isOpening = false;
float panelWidthTarget = 350.0f;
float panelWidthCurrent = panelWidthTarget;
float animationSpeed = 20.0f;
bool isKeyboardToggle = false;
float globalRotationSpeed = 4.0f;
float displaySpeed = 0.0f;
bool isStopping = false;
bool isSimulationRunning = false;
bool isButtonPressed = false;
int pressedButton = -1;
float buttonDepressAmount = 0.0f;
int pressedGearButton = -1;
int pressedRackButton = -1;
float gearButtonDepress = 0.0f;
float rackButtonDepress = 0.0f;
bool isFullscreen = false;
bool showHintsPanel = true;
float hintsPanelHeight = 120.0f;
bool isSoundPlaying = false;
bool shouldPlaySound = false;
float powerButtonX = 0.0f;
float powerButtonY = 0.0f;
float powerButtonSize = 25.0f;
bool isPowerButtonPressed = false;
float powerButtonDepress = 0.0f;
int hoveredGear = -1;
int mouseX = 0, mouseY = 0;
bool showTooltip = true;
bool isSoundEnabled = true;
float soundButtonX = 0.0f;
float soundButtonY = 0.0f;
float soundButtonSize = 25.0f;
bool isSoundButtonPressed = false;
float soundButtonDepress = 0.0f;


// Gear properties
GearProperties gear1Props = {20, 0.1f, 0.4f, 0.05f, 0.1f, 0.15f};
GearProperties gear2Props = {16, 0.1f, 0.3f, 0.05f, 0.1f, 0.15f};
GearProperties gear3Props = {30, 0.1f, 0.6f, 0.05f, 0.1f, 0.15f};
GearProperties gear4Props = {20, 0.1f, 0.4f, 0.05f, 0.1f, 0.15f};

// Rack properties
RackProperties rackProps = {8.0f, 0.15f, 0.05f, 0.15f, 0.15f, 0.2f, 0.02f};
...
void initGearPositions() {
    gearPositions.push_back({-1.0f, 0.72f, 3.0f, gear1Props.outerRadius, 1}); // Bánh rang 1
    gearPositions.push_back({-0.4f, 0.72f, 2.4f, gear2Props.outerRadius, 2}); // Bánh rang 2
    gearPositions.push_back({0.6f, 0.72f, 2.1f, gear3Props.outerRadius, 3});  // Bánh rang 3
    gearPositions.push_back({1.3f, 0.72f, 3.0f, gear4Props.outerRadius, 4});  // Bánh rang 4
}
...
glPushMatrix();
    ...
    glPushMatrix();
        ...
        drawCylinder(0.2f, 1.0f, 200, 200);
...
glPushMatrix();
    glTranslatef(rackPositionX - 2.23f, -2.0f, 2.2f);
    drawRack(rackProps);
...
if (showTooltip) { // Thêm ?i?u ki?n này
        drawGearTooltip();
    }
...
if (x >= uiMargin +60  && x <= uiMargin + 230 &&
    y >= controlButtonY +330 && y <= controlButtonY + 370) {
    	playClickSound();  // Thêm d?ng này
    ...
    return;
                ...
                if (x >= uiMargin  && x <= uiMargin + 120 &&
                    y >= controlButtonY+280 && y <= controlButtonY + 320) {
                    	playClickSound();  // Thêm d?ng này
                    isButtonPressed = true;
                    pressedButton = 0;
                    return;
                ...
                // ===== X? l? click nút RESET =====
                if (x >= uiMargin + 160 && x <= uiMargin + 290 &&
                    y >= controlButtonY + 280&& y <= controlButtonY + 320) {
                    	playClickSound();  // Thêm d?ng này
                    isButtonPressed = true;
                    pressedButton = 1;
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (x >= buttonX && x <= buttonX + buttonWidth+50 &&
                y >= buttonY+30 && y <= buttonY + buttonHeight+50) {
                	playClickSound();  // Thêm d?ng này
                pressedGearButton = i;
                return;
            }
        ...
        if (x >= uiMargin +60 && x <= uiMargin + 230 &&
            y >= rackButtonY +30 && y <= rackButtonY + 65) {
            	playClickSound();  // Thêm d?ng này
            pressedRackButton = 0;
            return;
        }
    ...
    if (y >= rackPanelY + 225 && y <= rackPanelY + 245) {
        ...
        return;
                ...
                if (selectedGear >= 0) {
                    ...
                    // Thay th? ph?n x? l? thanh tru?t t?c d? b?ng:
...
if (y >= gearPanelY + 105 && y <= gearPanelY + 125 && isRotating) {
    ...
    globalRotationSpeed = percent * 4.0f; // T?c d? t?i da chính xác là 4.0
    ...
    return;
                    ...
                    if (y >= gearPanelY + 165 && y <= gearPanelY + 185) {
                        ...
                        float percent = (x - sliderX) / sliderWidth;
                        percent = std::max(0.0f, std::min(1.0f, percent));
                        currentGear->thickness = 0.05f + percent * (0.3f - 0.05f);
                        return;
                    }
    ...
    if (isButtonPressed) {
        ...
        if (pressedButton == 0) { // Nút Start/Stop
        ...
        else if (pressedButton == 1) { // Nút Reset
            resetSimulation();
        }
    }
...
void motion(int x, int y) {
    ...
    if (isDraggingLeft && selectedProperty >= 0) {
        ...
        globalRotationSpeed = 0+ percent*4;
        ...
        // ========== X? L? THANH RANG ==========
        if (selectedProperty == 10) { // S? rang thanh rang
            rackTeethCount = 15 + percent * (35 - 15);
            rackProps.toothSpacing = rackProps.length / rackTeethCount;
        }
        else if (selectedProperty == 11) { // ?? dày thanh rang
            rackProps.thickness = 0.05f + percent * (0.5f - 0.05f);
        }
    ...
    glutPostRedisplay();
        ...
        globalRotationSpeed = percent * 4.0f;
    }
    ...
    break;
        ...
        if(cameraAngleX >= 170.0f) cameraAngleX= 170.0f;
        lastX = x;
        lastY = y;
        glutPostRedisplay();
    }
...
void keyboard(unsigned char key, int x, int y) {
    switch(key) {
        ...
        case 'D':
        case 'd':
            componentsAngleY -= 5.0f;
            if(componentsAngleY <= -45.0f) componentsAngleY = -45.0f;
            break;
        case 'A':
        case 'a':
            componentsAngleY += 5.0f;
            if(componentsAngleY >= 40.0f) componentsAngleY = 40.0f;
            break;
        case 'u':
        case 'U':
...
WIDTH = w;
HEIGHT = h;
uiPanelHeight = HEIGHT * 1.0f; // C?p nh?t chi?u cao c?a panel
...
void update(int value) {
    ...
    float deltaTime = (currentTime - lastTime) / 1000.0f; // Th?i gian th?c gi?a các frame
    ...
    if (isAnimating) {
        if (isOpening) {
            panelWidthCurrent = lerp(panelWidthCurrent, panelWidthTarget, 0.2f);
            if (panelWidthTarget - panelWidthCurrent < 1.0f) {
                panelWidthCurrent = panelWidthTarget;
                isAnimating = false;
                showUI = true;
                if (isKeyboardToggle) {
            ...
            panelWidthCurrent = lerp(panelWidthCurrent, 0.0f, 0.2f);
            if (panelWidthCurrent < 1.0f) {
                panelWidthCurrent = 0;
                isAnimating = false;
                showUI = false;
                if (isKeyboardToggle) {
    ...
    if (isButtonPressed) {
        ...
        buttonDepressAmount -= 1.5f; // T?c ?? tr? l?i nhanh h?n
        if (buttonDepressAmount <= 0) {
            buttonDepressAmount = 0.0f;
            pressedButton = -1;
        }
    }
