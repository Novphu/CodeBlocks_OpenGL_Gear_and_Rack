...
void drawPowerButton(float x, float y, float size, bool pressed)
{
    ...
    float offsetY = pressed ? powerButtonDepress : 0.0;
    ...
    glColor3f(0.9, 0.1, 0.1); // Màu ??
    glBegin(GL_TRIANGLE_FAN);
    for (int i = 0; i <= 360; i += 10)
    {
        float angle = i * M_PI / 180.0;
        glVertex2f(x + cos(angle) * (size * 0.8), y + offsetY + sin(angle) * (size * 0.8));
    }
    glEnd();
    ...
    glLineWidth(4.0);
    glBegin(GL_LINES);
    glVertex2f(x, y + offsetY - size * 0.4);
    glVertex2f(x, y + offsetY + size *0.05);
    glEnd();
    ...
    { // V? 3/4 v?ng tr?n
        float angle = i * M_PI / 180.0;
        glVertex2f(x + cos(angle) * (size * 0.4), y + offsetY + sin(angle) * (size * 0.4));
    }
...
void drawUI()
{
    ...
    if (!showUI && !isAnimating) {
        ...
        drawArrow(10, HEIGHT/2, 20.0, true); // M?i tên hu?ng ph?i (m? panel)
    ...
    // V? n?n panel
    glColor4f(0.1, 0.1, 0.1, 0.7 * fadeFactor);
    glBegin(GL_QUADS);
    glVertex2f(0, 0);
    ...
    {
        drawArrow(panelWidthCurrent-20, HEIGHT/2, 20.0, false); // M?i tên hu?ng trái (dóng panel)
    }

    // Ch? v? n?i dung khi panel m? d? l?n
    if (panelWidthCurrent > 50 && fadeFactor > 0.3)
    {
        ...
        glColor4f(1.0, 1.0, 1.0, fadeFactor);
    ...
    // Thân nút
    glColor4f(showHintsPanel ? 0.3 : 0.5, 0.3, 0.7, fadeFactor);
    glBegin(GL_QUADS);
    ...
    glColor4f(0.7, 0.7, 0.7, fadeFactor);
    glVertex2f(uiMargin + 60, toggleButtonCurrentY+10);
    glVertex2f(uiMargin + 230, toggleButtonCurrentY+10);
    glColor4f(0.4, 0.4, 0.4, fadeFactor);
    glVertex2f(uiMargin + 230, toggleButtonCurrentY + 50);
    glVertex2f(uiMargin + 60, toggleButtonCurrentY + 50);
    glEnd();
    glLineWidth(1.0);
    ...
    if (showHintsPanel && panelWidthCurrent > 100)
    {
        ...
        // N?n panel
        glColor4f(0.15, 0.15, 0.2, 0.8 * fadeFactor);
        glBegin(GL_QUADS);
        ...
        // Tiêu d? panel
        glColor4f(0.8, 0.8, 1.0, fadeFactor);
        ...
        // N?i dung g?i ?
        glColor4f(0.9, 0.9, 0.9, fadeFactor);
    ...
    glColor4f(isSimulationRunning ? 1.0 : 0.3,isSimulationRunning ? 0.3 : 0.7,isSimulationRunning ? 0.3 : 0.3,fadeFactor);
    ...
    glColor4f(0.3, 0.3, 0.7, fadeFactor);
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        float currentDepress = (pressedGearButton == i) ? gearButtonDepress : 0.0;
        ...
        if (i == selectedGear)
        {
            glColor4f(0.5, 0.7, 1.0, fadeFactor);
        }
        else
        {
            glColor4f(0.3, 0.3, 0.3, fadeFactor);
        }
    ...
    float rackButtonCurrentY = rackButtonY + 30 + (pressedRackButton >= 0 ? rackButtonDepress : 0.0);
    float rackButtonHeight = 35 - (pressedRackButton >= 0 ? rackButtonDepress/2 : 0.0);
    ...
    for (size_t j = 0; j < rackLabel.length(); ++j)
    {
        ...
        if (showRackPanel)
        {
            ...
            // N?n panel
            glColor4f(0.2, 0.2, 0.2, 0.8 * fadeFactor);
            glBegin(GL_QUADS);
        ...
        if (selectedGear >= 0 && showGearPanel)
        {
            ...
            if (isRotating || isStopping)
            {
                ...
                drawSlider(uiMargin + 10, gearPanelY + 110, panelWidthCurrent - 60,
                    displaySpeed/4.0 , "Speed: ", displaySpeed);
            }
            else
            {
            ...
            drawSlider(uiMargin + 10, gearPanelY + 140, panelWidthCurrent - 60,
                     (currentGear->numTeeth - 15) / 20.0, "Teeth: ", currentGear->numTeeth);

            drawSlider(uiMargin + 10, gearPanelY + 170, panelWidthCurrent - 60,
                     (currentGear->thickness - 0.05) / 0.25, "Thickness: ", currentGear->thickness);
...
void drawSlider(float x, float y, float width, float value, const char* label, float displayValue)
{
    ...
    if (strcmp(label, "Speed: ") == 0)
    {
        ...
        {
            glColor3f(0.3, 0.3, 0.3); // Màu xám khi d?ng
        }
        else
        {
            glColor3f(0.0, 0.7, 1.0); // Màu xanh duong b?nh thu?ng
        }
    ...
    if (strcmp(label, "Speed: ") == 0 && displayValue >= 4.0)
        {
        glColor3f(1.0, 0.0, 0.0); // Màu d?
    }
    else
    {
        glColor3f(0.0, 0.7, 1.0); // Màu xanh duong
    }
...
void drawSoundButton(float x, float y, float size, bool pressed, bool soundEnabled)
{
    ...
    // Tính toán v? trí khi b? nh?n
    float offsetY = pressed ? soundButtonDepress : 0.0;
    ...
    float speakerX = x - size * 0.3;  // D?ch sang trái
    float speakerSize = size * 1.0;   // Phóng to bi?u tý?ng

    if (soundEnabled)
    {
        // Bi?u tý?ng loa khi âm thanh b?t (l?n hõn và d?ch trái)
        glBegin(GL_LINE_STRIP);
        glVertex2f(speakerX - speakerSize*0.4, y + offsetY - speakerSize*0.2);
        glVertex2f(speakerX - speakerSize*0.4, y + offsetY + speakerSize*0.2);
        glVertex2f(speakerX, y + offsetY + speakerSize*0.2);
        glVertex2f(speakerX + speakerSize*0.4, y + offsetY + speakerSize*0.55);
        glVertex2f(speakerX + speakerSize*0.4, y + offsetY - speakerSize*0.55);
        glVertex2f(speakerX, y + offsetY - speakerSize*0.2);
        glVertex2f(speakerX - speakerSize*0.4, y + offsetY - speakerSize*0.2);
        glVertex2f(speakerX - speakerSize*0.1, y + offsetY - speakerSize*0.2);
        glVertex2f(speakerX - speakerSize*0.1, y + offsetY + speakerSize*0.2);
        glEnd();

        // V? 3 ðý?ng parabol n?m d?c
        float centerY = y + offsetY;  // Tâm theo tr?c Y
        float baseX = x + speakerSize*0.25; // V? trí b?t ð?u theo tr?c X
        float horizontalSpacing = speakerSize*0.2; // Kho?ng cách ngang

        // Thông s? cho 3 parabol (chi?u cao tãng d?n)
        float waveHeights[3] = {speakerSize*0.2,speakerSize*0.35,speakerSize*0.5};

        float waveWidths[3] = { speakerSize*0.15,speakerSize*0.2,speakerSize*0.25 };

        glColor3f(1.0, 1.0, 1.0);
        glLineWidth(2.5); // Làm ð?m hõn m?t chút
        ...
        for (int wave = 0; wave < 3; wave++)
        {
            ...
            glBegin(GL_LINE_STRIP);
            for (int i = 0; i <= 20; i++)
            { // Tãng s? ðo?n ð? mý?t hõn
                float t = (float)i/20 * 2.0 - 1.0;

                // V? trí Y (t? dý?i lên trên)
                float yPos = centerY + t * waveHeights[wave];

                // V? trí X theo parabol: x = a*(1 - t^2)
                float xPos = currentX + (1.0 - t*t) * waveWidths[wave];
    ...
    else
    {
        ...
        glLineWidth(3.0); // Làm ð?m d?u X
        glBegin(GL_LINES);
        // Ðý?ng chéo t? trái sang ph?i
        glVertex2f(speakerX + speakerSize*0.6, y + offsetY - speakerSize*0.3);
        glVertex2f(speakerX + speakerSize*1.1, y + offsetY + speakerSize*0.3);
